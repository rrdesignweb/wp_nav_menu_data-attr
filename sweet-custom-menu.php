...
class rc_sweet_custom_menu {
	...
	*/
	function rc_scm_add_custom_nav_fields( $menu_item ) {
	    ...
	    $menu_item->dataAttr = get_post_meta( $menu_item->ID, '_menu_item_dataAttr', true );
	    return $menu_item;
	...
	*/
	function rc_scm_update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {
	    ...
	    if ( is_array( $_REQUEST['menu-item-dataAttr']) ) {
	        $dataAttr_value = $_REQUEST['menu-item-dataAttr'][$menu_item_db_id];
	        update_post_meta( $menu_item_db_id, '_menu_item_dataAttr', $dataAttr_value );
	    }
